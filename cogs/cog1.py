...
class SimpleView(discord.ui.View):
    ...
    async def GiveAway(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user in self.ListofPeople:
            ...
            self.em.set_field_at(
                4, name="", value=f":large_blue_diamond: Number of Participants: {str(len(self.ListofPeople))}", inline=False)
...
async def CreateAccount(userid, matic, twitter):
    ...
    users[userid] = {"balance": 0,
                     "WalletAddress": matic, "TwitterAccount": twitter}
    with open("bank.json", "w") as f:
        json.dump(users, f)
...
class StartCog(commands.Cog):
    ...
    async def user_info(self, interaction: discord.Interaction, user: discord.User):
        ...
        if validuser:
            users = await getBankData()
            em = discord.Embed(
                title=f"{user.name}'s Info", color=discord.Color.yellow())
            Balance = users[str(user.id)]["balance"]
            Address = users[str(user.id)]["WalletAddress"]
            Twiiter = users[str(user.id)]["TwitterAccount"]
            em.add_field(
                name="", value=f"User's Balance: {Balance}", inline=False)
            em.add_field(
                name="", value=f"User's ETH Address: {Address}", inline=False)
            em.add_field(
                name="", value=f"User's Twiiter Account: {Twiiter}", inline=False)
    ...
    async def buy_item(self, interaction: discord.Interaction, shop_item: Choice[int]):
        ...
        if UserBalance < shop_item.value:
            ...
            if (shop_item.name == "RAFFLE TICKET 🎟️"):  # Add role logic
                RaffleRole = discord.utils.get(
                    interaction.guild.roles, name="Raffle Ticket")
    ...
    async def leaderboard(self, interaction: discord.Interaction):
        users = await getBankData()
        em = discord.Embed(title="Doonz Coin Leaderboard",
        ...
        for i, entry in enumerate(Leaderboard[:10], start=1):
            em.add_field(
                name="", value=f"{i}- <@{entry[1]}> ({entry[0]})", inline=False)
        await interaction.response.send_message(embed=em)
    ...
    async def add_money(self, interaction: discord.Interaction, send_to: discord.User, amount: str):
        ...
        em = discord.Embed(title="Doonz Coin Leaderboard",
                           color=discord.Color.yellow())
        em.add_field(
            name="", value=f"added {amount} :moneybag: to <@{send_to.id}>")
        await interaction.response.send_message(embed=em)
